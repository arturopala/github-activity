module GitHubIssueCommentEventPayloadTests exposing (all)
...
all : Test
all =
    describe "decodeIssueCommentEventPayload should"
        [ test "decode valid IssueCommentEvent json" <|
            \() ->
                let
                    underTest =
                        decodeString GitHub.Decode.decodeIssueCommentEventPayload
                            """{
                                 "action": "created",
                                 "issue": {
                ...
                underTest
                    |> Expect.all
                        [ expectOk errorToString
                            [ having .action (Expect.equal "created")
                            , having .issue
                                (Expect.all
                                    [ having .id (Expect.equal 444500041)
                                    , having .title (Expect.equal "Spelling error in the README file")
                                    ]
                                )
                            , having .comment
                                (Expect.all
                                    [ having .id (Expect.equal 492700400)
                                    , having .body (Expect.equal (Just "You are totally right! I'll get this fixed right away."))
                                    ]
                                )
                            ]
                        ]
